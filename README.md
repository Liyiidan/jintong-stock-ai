# 金通科技
### 基于 LLM + MoT 架构的多维度 AI 智能股票分析与量化决策系统
...
以下截图来自 `jintong-real` Docker 环境的实际运行页面，展示的是当前真实数据工作台的界面状态。截图中的行情、数据源和报告日期会随本地数据库同步时间变化；它们不是前端静态 Mock 页面。

### 首页与能力状态

![金通科技首页](docs/screenshots/home.png)

首页集中展示系统能力、真实数据状态、数据源和最近同步时间，用户可以先确认当前实例是否具备行情、财务和智谱 AI 能力。

### 选股评审

![选股评审页面](docs/screenshots/discover.png)

选股评审页提供快照类型、数据日期、排名数量、股票筛选、五专家分项评分和冲突信号入口。

### 000001 单股分析

![000001 平安银行单股分析页面](docs/screenshots/stock-000001.png)

单股页面将 K 线、成交量、来源和更新时间放在分析入口，后续展开专家结论、证据、AI 状态、风险和交易执行方案。

### 宏观分析

![宏观面独立分析页面](docs/screenshots/macro.png)

宏观页展示报告日期、新闻覆盖数量、指数数据可用性和缺失字段，避免把缺失数据渲染成看似精确的结论。
...
这个版本不只是页面原型，而是我在原团队项目基础上持续完成的真实数据化、前端重构、稳定性和可观测性改造：
...
- 对原有前端进行了整体重构：重新组织路由、页面层级、分析结果布局、数据状态反馈和响应式样式，替换原先偏粗糙的展示界面，使首页、选股评审、单股分析、宏观分析、持仓和投资者画像形成统一的工作台体验。
- 重做单股分析页的信息层级和交互：加入可拖拽/缩放 K 线、选中交易日详情、来源状态面板、AI 状态横幅、专家卡片和结构化投资执行区，让用户能从数据到结论逐层核验。
- 重做认证、画像和空状态交互，补充无画像注册、加载中、失败、无数据和重新分析等边界状态，避免前端只展示静态结果。
